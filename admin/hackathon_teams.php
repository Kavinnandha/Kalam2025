...
<body class="bg-gray-100 min-h-screen">
    ...
    {
        $sql = "SELECT h.*, ht.user_id, ht.team_member_id, u.name, u.email, u.phone, h.project_file
                FROM hackathon h
        ...
        <div class="grid grid-cols-1 gap-8">
                                ...
                                <?php foreach ($teams as $team): ?>
                                    <div
                                                ...
                                                <div class="space-y-3">
                                                    <?php foreach ($team['members'] as $member): ?>
                                                        <div class="flex items-start text-sm border-b border-gray-100 pb-2">
                                                            <div
                                                                class="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-700 flex-shrink-0 mr-3">
                                                                <?php echo substr($member['name'] ?? $member['user_id'], 0, 1); ?>
                                                            </div>
                                                            <div class="flex-grow">
                                                                <p class="font-medium">
                                                                    <?php echo htmlspecialchars($member['name'] ?? 'N/A'); ?></p>
                                        ...
                                        <div class="px-4 py-3 border-t border-gray-100 bg-gray-50">
                                            <?php if (!empty($team['project_file']) && file_exists($team['project_file'])): ?>
                                                <div class="flex justify-between items-center">
                                                    ...
                                                    <div class="flex space-x-2">
                                                        <a href="view_pdf.php?file=<?php echo urlencode($team['project_file']); ?>&action=view"
                                                            class="inline-flex items-center px-3 py-1 bg-indigo-100 text-indigo-700 rounded-md text-xs hover:bg-indigo-200 transition-colors"
                                                            target="_blank">
                                                            <i class="fas fa-eye mr-1"></i> View
                                                        </a>
                                                        <!--
                                                        <a href="view_pdf.php?file=<?php echo urlencode($team['project_file']); ?>&action=download"
                                                            class="inline-flex items-center px-3 py-1 bg-gray-100 text-gray-700 rounded-md text-xs hover:bg-gray-200 transition-colors">
                                            ...
                                            <?php else: ?>
                                                <p class="text-sm text-gray-500 flex items-center">
                                                    ...
                                                    No project Presentation uploaded
                                                </p>
                                            <?php endif; ?>
                                        </div>
                                    </div>
            ...
            <?php if (count($events) === 0): ?>
                <div class="text-center py-12">
                    ...
                    <?php if ($department_code): ?>
                        <p class="text-gray-500 mt-2">No events are currently registered for department:
                            <?php echo htmlspecialchars($department_code); ?></p>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
